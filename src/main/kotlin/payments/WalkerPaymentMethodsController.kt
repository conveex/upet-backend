...
class WalkerPaymentMethodsController(
    private val repository: WalkerPaymentMethodRepository
) {
    ...
    suspend fun listMyMethods(call: ApplicationCall, principal: JWTPrincipal?) {
        ...
        if (!isWalker) {
            call.respond(
                ...
                UserPaymentMethodsEnvelope(
                    success = false,
                    message = "Solo los paseadores pueden gestionar métodos de pago de paseador."
                )
            )
            return
        }
        ...
        call.respond(
            ...
            UserPaymentMethodsEnvelope(
                success = true,
                message = "Métodos de pago del paseador.",
                methods = methods
            )
        )
    }
